    ...
    if (id) {
      this.lib.db
        ...
        .exec((err, data) => {
          ...
          if (!data)
            return next(
              this.RESTError("ResourceNotFoundError", "Store not found")
            );
  ...
  storeBooksales(req, res, next) {
    ...
    if (id) {
      this.lib.db
        ...
        .populate("books")
        .exec((err, data) => {
          if (err) return this.RESTError("InternalServerError", err);
          this.writeHAL(res, data);
        });
    } else {
      next(this.RESTError("InvalidArgumentError", "Invalid id"));
    ...
    if (id) {
      this.lib.db
        ...
        .exec((err, store) => {
          ...
          if (!store)
            return next(
              this.RESTError("ResourceNotFoundError", "Store not dound")
            );
  ...
  controller.addAction(
    {
      ...
      params: [
        swagger.queryParam(
          "state",
          "Filter the list of stores by state",
          "string"
        ),
      ],
      responseClass: "Store",
  ...
  controller.addAction(
    {
      ...
      method: "GET",
      params: [
        swagger.pathparam("id", "The id of the store", "string"),
  ...
  controller.addAction(
    {
      ...
      params: [
        swagger.bodyParam("store", "The JSON data of the store", "string"),
      ],
      responseClass: "Store",
      nickname: "newStore",
    },
